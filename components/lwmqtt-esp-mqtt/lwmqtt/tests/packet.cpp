...
TEST(Packet, EncodeConnectPasswordOnly) {
  uint8_t pkt[22] = {
      LWMQTT_CONNECT_PACKET << 4u, 20, 0, 4, 'M', 'Q', 'T', 'T', 4, 194, 0, 60, 0, 0, 0, 0, 0, 4, 'p', 'a', 's', 's',
  };
...
TEST(Packet, DecodeAckInvalidPubrelFlags) {
  uint8_t pkt[4] = {
      (LWMQTT_PUBREL_PACKET << 4u), 2,
      0,
      7,
  };

  uint16_t packet_id = 0;
  lwmqtt_err_t err = lwmqtt_decode_ack(pkt, sizeof(pkt), LWMQTT_PUBREL_PACKET, &packet_id);

  EXPECT_EQ(err, LWMQTT_MISSING_OR_WRONG_PACKET);
}

TEST(Packet, DecodeAckInvalidPubackFlags) {
  uint8_t pkt[4] = {
      (LWMQTT_PUBACK_PACKET << 4u) | 0x02u, 2,
      0,
      7,
  };
...
TEST(Packet, DecodePublishInvalidQOS) {
  uint8_t pkt[22] = {
      (LWMQTT_PUBLISH_PACKET << 4u) | 0x06u,
      20,
      0,
      6,
      'l',
...
TEST(Packet, DecodePublishInvalidPacketId) {
  uint8_t pkt[24] = {
      LWMQTT_PUBLISH_PACKET << 4u | 2,
      22,
      0,
      6,
      'l',
...
TEST(Packet, DecodeSubackOverflow) {
  uint8_t pkt[6] = {
      LWMQTT_SUBACK_PACKET << 4u, 4, 0, 7, 0, 1,
  };

  uint16_t packet_id;
  int count = 0;
  lwmqtt_qos_t granted_qos_levels[1];
  lwmqtt_err_t err = lwmqtt_decode_suback(pkt, sizeof(pkt), &packet_id, 1, &count, granted_qos_levels);

  EXPECT_EQ(err, LWMQTT_SUBACK_ARRAY_OVERFLOW);
}
